...
async function getRecipes() {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const url of RECIPE_URLS) {
      // A5. try / catch around the async network work
      try {
        // A6. Fetch the URL
        const response = await fetch(url);
        // A7. Retrieve JSON from the response
        const recipe = await response.json();
        // A8. Add the new recipe to the array
        recipes.push(recipe);
        // A9. If we've finished retrieving all recipes, save and resolve
